...
class RunStreaming extends RunJob {
  ...
  object ConversionUtils extends Serializable {
    ...
    def cleanTweet: (String => String) = { line: String =>
      var new_tweet: String = ""
      new_tweet = line.replaceAll("RT [@]\\w*:\\s", "")
        .replaceAll("#", "")
        .replaceAll(",", "")
        .replaceAll("\n", "")
        .replaceAll("|", "")
    ...
    def getTelRegex: (String => Array[String]) = { line: String =>
      val pattern ="""(?:\+33|0033|0) ?[0-8](?:[\. \-]?[\d]{2}){4}""".r
      // val test_text = "Mon numéro de téléphone est le 01.47.60.28.29, et mon numéro de portable est le +33685194709"
    ...
    val getTelRegexUdf = udf(getTelRegex)


    def extractMentionned: (String => List[String]) = { line: String =>
      val tweet = line.replaceAll("^[@]\\w*\\s", "")
      val pattern = "[@]\\w*".r
      val arrayMention: List[String] = pattern.findAllIn(tweet).toList.map(mention => mention.replaceAll("@", ""))
      arrayMention
    }

    val extractMentionnedUdf = udf(extractMentionned)
